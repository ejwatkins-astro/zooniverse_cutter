...
def hex_to_hsv(hexstring):
    ...
    rgb = np.array(hex_to_rgb(hexstring))/255. #Convert Hex to RGB fracs (i.e., 0..1 instead of 0..255)
    maxc = max(rgb) # v is the max colour value
    minc = min(rgb)

    if minc == maxc: #i.e all zero
        return 0.0, 0.0, maxc

    s = (maxc - minc) / maxc

    rc_gc_bc = (maxc - rgb) / (maxc - minc)

    ind = np.argmax(rgb)
    h = 2.0 * ind + np.diff(np.roll(rc_gc_bc, ind * -1)[1:])[0]
    ...
    h = (h / 6.0) % 1.0
    return h, s, maxc #All in fractions in range [0...1]

def hsv_image_to_rgb(hsv):


    vtpq = np.zeros((*hsv.shape[:-1], 4))
    vtpq[:,:,0] = hsv[:,:,2] # v
    hsv = np.delete(hsv, 2, axis=2) #managing memory
    ...
    f = 6 * hsv[:,:,0] - h_floor
    h_floor = None
    hsv = np.delete(hsv, 0, axis=2)[:,:,0]

    vtpq[:,:,1] = vtpq[:,:,0] * (1 - (1 - f) * hsv) #s

    vtpq[:,:,2] = vtpq[:,:,0] * (1 - hsv) #p, p=minc #s
    vtpq[:,:,3] = vtpq[:,:,0] * (1 - f * hsv) # q #s
    hsv = None
    # print(h_int[4000,4000:4010])
    for i in range(7):
        # try:
        test = h_int == i
        if test.all():
            vtpq = np.delete(vtpq,-1 + i * 2%6, axis=2) #loops through -1(=3), 1, 3
            vtpq = np.roll(vtpq, i -1 * (i//2), axis=2) # 0, 1, 1, 2, 2, 3 (3=0)
            vtpq *= 255 #now rgb
            break
        #should break before getting here. If it doesnt, it is likely due to very low
        #hues?
        if i == 6:
            raise ValueError("So will need to do more intesive/slower code if this ever occurs. ")
        # except ValueError:
        #     return h_int

    # h_int_0 = where(h_int==0)
    # h_int_1 = where(h_int==1)
    return vtpq.astype(int)
...
def colorize_image(image, colorvals, colorintype='hsv', dtype=np.float, gammacorr_color=1):
    ...
    # hsv = ski_color.rgb2hsv(image)#.astype(dtype) # dont need this...
    hsv = np.zeros_like(image)
    hsv[:,:, 2] = image[:,:,2]
    image = None #memory help
    if colorintype.lower() == 'rgb':

        colorvals = np.array(hex_to_hsv(rgb_to_hex(colorvals)))#.astype(dtype)

    elif colorintype.lower() == 'hex':
        colorvals = np.array(hex_to_hsv(colorvals))#.astype(dtype)
    ...
    return ski_color.hsv2rgb(hsv)#.astype(dtype)#hsv_image_to_rgb(hsv)#
...
def combine_multicolor(im_list_colorized,gamma=2.2,inverse=False):
    ...
    if inverse:
        RGB_maxints = 1 - RGB_maxints
    scale_ratio = RGB_maxints/np.max(RGB_maxints)

    for i in range(3):
        combined_RGB[:,:,i] = np.nan_to_num(rescale_intensity(combined_RGB[:,:,i], out_range=(0, scale_ratio[i])))
